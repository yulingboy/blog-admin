...
<script>
import api from '@/api/sentence.js';
...
export default {
  ...
  methods: {
    getSentencesList() {
      api.getSentences(this.pageInfo).then(res => {
        console.log(res);
        if (res.meta.status === 200) {
          this.data = res.data.sentence;
    ...
    async clickButton(val) {
      ...
      // 修改
      if (val.methods === 'edit') {
        await api.getSentence(val.row._id).then(res => {
          console.log(res);
          if (res.meta.status === 200) {
      ...
      if (val.methods === 'del') {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          .then(async () => {
            await api.deleteSentence(val.row._id);
            this.$message({
              type: 'success',
              message: '删除成功!'
            });
